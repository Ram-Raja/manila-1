...
class ShareInstanceAccessTestCase(test.TestCase):
    ...
    def test_update_access_rules_maintenance_mode(self, maintenance_mode):
        rule1 = db_utils.create_access(
            id='fakeid1',
            share_id=self.share['id'],
            access_to='fakeip1',
        )
        rule2 = db_utils.create_access(
            id='fakeid2',
            share_id=self.share['id'],
            access_to='fakeip2',
        )
        ...
        delete_rules = [rule2]
        rules = [rule for rule in original_rules if rule not in delete_rules]
        if maintenance_mode:
        ...
        if maintenance_mode:
            ...
            self.driver.update_access.assert_called_once_with(
                self.context, share_instance, rules, add_rules=[],
                delete_rules=delete_rules, share_server=None
            )
